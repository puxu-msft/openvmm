...
| 7 | `poc7_rpm_vmwp.ps1` / `poc7b_livekd_dump.ps1` | **host 进程**经 RPM(vmwp)/livekd hypercall **读 guest RAM** —— 纠之前过宽的"host 够不到"结论；oracle = GPA 0x100000 处 marker 0x9E66...9E66 | 真 OpenHCL VM (admin) | **7a (RPM)** ❌：OpenProcess OK 但 vmwp 总 commit 仅 32 MiB、扫不到 marker → **(d) OpenHCL 不映 guest RAM 进 vmwp VA**（LiveCloudKd 2010 模型不适用）。**7b (livekd)** ❌：`VidGetPartitionIds` 立即被拒（vid.sys EPROCESS 门禁；livekd 不是 vmwp）；补 kd.exe 重跑同根因。**两轮对抗 subagent 穷尽 12 条路径全否定 live host 访问**，仅 B1 frozen snapshot 可行。|
| B1 | `Save-VM → .vmrs` + `docs/superpowers/examples/vmrs_log_scanner/` | host 经 saved-state 文件读 guest 物理内存（OpenHCL 实测过；frozen snapshot 非 live）| 真 OpenHCL VM | ✅ **已有现成工具**（vmrs_log_scanner），但仅 oracle/取证用途，**不可作 live data plane** |
