...
### 4.14 真 WS2025 Windows inbox interop 打通（static controller model，C1+C2+C3，2026-06-14）

**来源**：母项目 windows-nvme-controller-emulator 的 Windows NVMe-oF 互通手册（2026-06-13 交接）暴露一个 usnvmemu 在**纯 Linux 环境结构性看不见**的盲区 = **static controller model**：Windows inbox initiator（`stornvmeofi.sys` / `nvmeofutil`）默认 `connect -ci`（static specific CNTLID），Linux `nvme-cli` 默认 dynamic（`0xFFFF`）。usnvmemu 团队自建 WS2025 Hyper-V VM（克隆兄弟 VM 的 guest.vhdx；Hyper-V Default Switch NAT + Python asyncio TCP relay 转 WSL2 target）跑 real Windows 真连。

**🎉 真机 e2e PROVEN**：真 WS2025 → Hyper-V relay → usnvmemu target，`Get-Disk` 显 **"NVMe OpenHCL Userspace NVMe v2.0" Online / 67108864 bytes（64 MiB）**，raw 4KB write+read **byte-equal**。**推翻手册「RDMA-only」倾向 = Windows inbox 支持 TCP 实证**；坐实 static model 默认（验 C1）。三处 transport-无关修复（都因 Linux nvme-cli 默认行为而纯 Linux 永远盖不到）：

- **C1（commit `e2c022575`）— Connect CNTLID 校验 + SCT 修复**：`dispatch_plan::decide_connect_cntlid` 纯函数（sync/async dual-track）：dynamic(`0xFFFF`)/static-any(`0xFFFE`)/具体匹配 → accept；指定不存在的具体 cntlid → reject SC=0x82 + CQE result DW0 = IPO/IATTR(`0x0001_0004`)，修「静默 coerce 成 1」。**SCT 修复（CRITICAL，architect 发现）**：fabrics SC(0x80-0x9F) 原用 SCT=0x07 Vendor Specific 是既存 wire bug，应 SCT=0x01 Command Specific（IPO/IATTR 仅此下定义；独立 oracle：Linux `nvme.h` `NVME_SC_CONNECT_*`=`0x180`-`0x184`=SCT0x1|SC0x80-0x84 确证）；连带修 self-consistent 错误测试——又一次「self-consistent ≠ spec-conformant」（见 LESSONS §33）。
- **C2（commit `9f1ae4d87`）— discovery static-model 广告**：`DiscoveryPortal.cntlid`（默认 0xFFFF dynamic）+ bin `--discovery-static-cntlid <N>` → static-default host 经 `nvme discover` 学到具体 CNTLID。
- **C3（commit `4df5c3f83`）— transport-SGL Connect data via R2T（真连深挖才暴露的根因）**：relay byte-log 解码坐实 WS2025 IO-queue Fabric Connect 与 admin 不同——admin Connect data 走 in-capsule（SGL `0x01`），但 **IO Connect data 走 Transport SGL（`sqe[39]=0x5A` = type 0x5/subtype 0xA），CapsuleCmd 只含 64B SQE、无 in-capsule data → 须发 R2T、host 回 H2CData 推 1024B**。根因链：[cmd.rs:769](/usnvmemu/crates/nvme_firmware/src/cmd.rs) `IOCCSZ=4`（IO 胶囊=64B SQE，不支持 in-capsule data，spec-consistent）→ Windows 正确改用 transport SGL → 但旧 `handle_connect` 在 parse 前 `if data.len()!=1024 { reject }` 拒掉、**从不发 R2T**（内部不一致：广告「IO 无 in-capsule」却要求 Connect data in-capsule）。修复（保 IOCCSZ=4、不退化加 in-capsule，ADR-008）：`fabric::decode_connect_data_source` 路由（InCapsule/ViaR2t/Invalid）+ R2T fetch；连带处理 **Windows H2CData PLEN=HLEN quirk**（Windows 把 CommonHdr PLEN 设为 HLEN、不计 data，违 NVMe/TCP transport binding（TP-8000），data 长由 PSH DATAL 给）= `await_host_data_async` 据 DATAL 补读 + 全 R2T 读加 30s inactivity timeout（DoS bound）。regression gate `tests/vt_windows_transport_sgl_connect.rs`（3 测，精确 Windows wire：SGL 0x5A + H2CData PLEN=HLEN + 越界拒）。

**验证**：176 lib + 全 integration（含 3 transport-SGL）+ 4 probe + discovery static-advertise 全绿，clippy clean，architect PASS-with-fixes（PLEN quirk 非-conformant 确认 + timeout 已补）+ rust-reviewer APPROVE。证据本位回应 [usnvmemu-response-to-ws-handbook.md](/usnvmemu/experiments/2026-06-13-openhcl-vpci-nvme/usnvmemu-response-to-ws-handbook.md)。plan [2026-06-14-connect-transport-sgl-data.md](/usnvmemu/crates/nvme_of_tcp_target/docs/plans/2026-06-14-connect-transport-sgl-data.md) + [2026-06-14-static-controller-model-conformance.md](/usnvmemu/crates/nvme_of_tcp_target/docs/plans/2026-06-14-static-controller-model-conformance.md)。

**deferred → ROADMAP `V-spec-strict-mode`**：① spec-strict reject `cntlid=0`；② IO queue Connect 拒哨值。real host 不触发，纯 spec 纯化。DHCHAP DH-group 缺口（VM 未配 in-band auth 未触发）+ RDMA 路（VM 内有驱动但无 RDMA NIC，V9 独立成线）留后。C1/C2 transport-无关 fabric 修复将被 V9 RDMA 继承。
