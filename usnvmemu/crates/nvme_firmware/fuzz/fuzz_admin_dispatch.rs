...
fn run(input: AdminDispatchInput) {
    ...
    let (mut c, mut cap, _img) = match common::open_tmpfile_controller("admin") {
        Some(t) => t,
        None => return,
    };
    ...
    // O(N) DMA-drain（见 common::drive_dma_drain）：服务所有 DMA——DmaRead = Get Log Page 大
    // payload 的 PRP-list 页 fetch、DmaWrite = payload 写 host/CQE-post（喂空 ok=true，helper 自动；
    // 服务 write 是 op 表 drain oracle 前提，否则留 in-flight write 致误判泄漏）。fetch 逻辑见 feed。
    common::drive_dma_drain(
        ...
        |_token, len, idx| {
            let i = idx as usize;
            let mut bytes = if i < input.fed_pages.len() {
                let page = &input.fed_pages[i];
