                ...
                match crate::controller::completion::pi_read_verify_split(
                    ...
                    Err((other, lba_i)) => {
                        ...
                        tracing::warn!(
                            ...
                            lba = lba_i,
                            ?other,
                            "SGL PI READ: stored PI verify 失败"
                        );
                        return Some(Cqe::error(
                            cid,
                            sq_id,
        ...
        let (pi_finalize, pract) = match &pi {
            Some(pa) => {
                let layout = if pa.inline && !pa.pract {
                    ...
                    crate::controller::PiLayout::Separate {
                        mptr: pa.mptr,
                        meta: None, // PRACT=0 separate 经 MPTR DMA-read 填；PRACT=1 不用（自算）。
                        meta_pending: false,
                    }
                };
                let pf = crate::controller::PiFinalize {
    ...
    pub(super) fn dispatch_io(
        ...
        match sqe.opcode() {
            nvm_opc::READ => {
                ...
                // **Phase Q1 + B6b/B6c PRACT=0 路径分流**:
                // - PRACT=1 + 非 PI NS = INVALID_PROTECTION_INFO（spec § 8.3.1）
                // - PRACT=0 + PI NS：按 NS metadata 形态分两路：
                //     · separate (meta_inline=false)：B6b host data→PRP / PI→MPTR；
                //     · inline (meta_inline=true)：B6c host 经 dual-PRP 提供完整
                //       extended block（PRP1=4096 + PRP2=8，nlb=1），nlb≥2 需 PRP-list 待续。
                if pract && !is_pi_path {
                ...
                // **SGL×PI P-B/P-C** — SGL data path on PI NS（separate-meta P-B / inline-meta
                ...
                if is_sgl && is_pi_path {
                    ...
                    let pi_sector = if meta_inline_r && !pract {
                        ns.block_bytes() as u64
                    } else {
                        ns.data_bytes() as u64
                    };
                ...
                if !pract && is_pi_path {
                    ...
                    if meta_inline_r {
                            ...
                            == crate::controller::prp::PrpTier::List
                        {
                            ...
                            // **#4c** PRP1 可带偏移；PRP2（list 页指针）须页对齐。
                            if (prp2 & (NVME_PAGE_SIZE - 1)) != 0 {
                                tracing::warn!(
                                    "inline-meta READ nlb≥2：PRP2（list 页指针）须页对齐 → PRP_OFFSET_INVALID"
                                );
                                return Some(Cqe::error(
                                    cid,
                                    sq_id,
                            ...
                            // 全 OK → 连续 extended-block 流按 PRP 段切（首段含 PRP1 偏移），
                            // 走 plain PRP-list scatter。
                            let prp_off = crate::controller::prp::prp1_offset(prp1);
                            let total_pages =
                                crate::controller::prp::total_pages(prp_off, total_bytes);
                            let mut data_pages: Vec<Option<Vec<u8>>> =
                                Vec::with_capacity(total_pages as usize);
                            let mut off = 0usize;
                            for p in 0..total_pages {
                                let sz = crate::controller::prp::page_size(p, prp_off, total_bytes)
                                    as usize;
                                let end = (off + sz).min(blocks.len());
                        ...
                        if block_bytes != NVME_PAGE_SIZE as usize + 8 {
                            tracing::warn!(
                                block_bytes,
                                "inline-meta READ 仅支持标准 PI NS（block_bytes=4104）"
                            );
                            return Some(Cqe::error(cid, sq_id, sq_head, phase, sc::INVALID_FIELD));
                        }
                        ...
                        if let Err(e) = ns_mut.read_at(&mut block, slba * block_bytes as u64) {
                            tracing::warn!(error = %e, nsid, slba, "inline-meta READ backing fail");
                            return Some(Cqe::error(
                                cid,
                                sq_id,
                                sq_head,
                        ...
                        // 按 pi_first 切 data / tuple，verify stored PI（按 PRCHK 门控）。
                        let (data_slice, tuple_slice) = if pi_first {
                        ...
                        let stored = crate::pi::PiTuple::from_bytes(&tuple_arr);
                        let prchk = crate::pi::PrChk::from_cdw12(cdw12);
                        match stored.verify(data_slice, slba, pi_type, prchk) {
                            crate::pi::PiCheck::Ok => {}
                            other => {
                                // **reviewer L-2**：PiCheck::Ok 已在上面 arm 排除，to_sc()
                                // 对所有其他变体单射返 Some；用 expect 表达不变量，比
                                // unwrap_or(0x82) 默认值更诚实（默认值不可达，但读者难辨）。
                                let sc_byte = other
                                    .to_sc()
                                    .expect("non-Ok PiCheck always maps to an SC byte");
                                tracing::warn!(
                                    ...
                                    slba,
                                    ?other,
                                    "inline-meta READ: stored PI verify 失败"
                                );
                                return Some(Cqe::error(
                                    cid,
                                    sq_id,
                        ...
                        self.inline_meta_reads.insert(
                            op_id,
                            crate::controller::InlineMetaReadAccum {
                                sq_id,
                                cid,
                                sq_head,
                                cq_id,
                        ...
                        == crate::controller::prp::PrpTier::List
                    {
                        ...
                        if sqe.mptr == 0 {
                            tracing::warn!(nsid, "B6b-4 N>2 READ 需 MPTR");
                            return Some(Cqe::error(cid, sq_id, sq_head, phase, sc::INVALID_FIELD));
                        }
                        if prp2 == 0 {
                        ...
                        let total_lba = ns.total_lba;
                        if block_bytes != NVME_PAGE_SIZE as usize + 8
                            || data_bytes != NVME_PAGE_SIZE as usize
                        {
                            tracing::warn!(
                                block_bytes,
                                "B6b-4 READ 仅支持标准 PI NS（block=4104, data=4096）"
                            );
                            return Some(Cqe::error(cid, sq_id, sq_head, phase, sc::INVALID_FIELD));
                        }
                        ...
                        // reviewer M-1（B6b-4 N>2）：与 WRITE 分支统一用 checked_add，防御
                        // slba+nlb 溢出（受 MDTS+total_lba 约束实际不可达，但一致性/防御优先）。
                        match slba.checked_add(nlb as u64) {
                            Some(end) if end <= total_lba => {}
                            _ => {
                                return Some(Cqe::error(
                        ...
                        // 读 N 个 interleaved block。
                        let mut blocks = vec![0u8; block_bytes * nlb as usize];
                        let ns_mut = self.ns_mut(nsid).unwrap();
                        if let Err(e) = ns_mut.read_at(&mut blocks, slba * block_bytes as u64) {
                            tracing::warn!(error = %e, nsid, slba, "B6b-4 READ backing fail");
                            return Some(Cqe::error(
                                cid,
                                sq_id,
                                sq_head,
                        ...
                        // 拆 N data + N tuple，verify-all stored PI（按 PRCHK 门控）。
                        let prchk = crate::pi::PrChk::from_cdw12(cdw12);
                        let mut data_pages: Vec<Vec<u8>> = Vec::with_capacity(nlb as usize);
                        let mut tuple_concat: Vec<u8> = Vec::with_capacity(nlb as usize * 8);
                        for i in 0..nlb as usize {
                            let blk = &blocks[i * block_bytes..(i + 1) * block_bytes];
                            let (data_slice, tuple_bytes): (&[u8], [u8; 8]) = if pi_first {
                            ...
                            let stored = crate::pi::PiTuple::from_bytes(&tuple_bytes);
                            match stored.verify(data_slice, slba + i as u64, pi_type, prchk) {
                                crate::pi::PiCheck::Ok => {}
                                other => {
                                    let sc_byte = other
                                    ...
                                    tracing::warn!(
                                        ...
                                        lba = slba + i as u64,
                                        ?other,
                                        "B6b-4 READ: stored PI verify 失败"
                                    );
                                    return Some(Cqe::error(
                                        cid,
                                        sq_id,
                            ...
                            data_pages.push(data_slice.to_vec());
            ...
            nvm_opc::WRITE => {
                ...
                // **Phase Q1 + B6b/B6c PRACT=0 路径分流（WRITE）**:
                // - PRACT=1 + 非 PI NS = INVALID_PROTECTION_INFO（spec § 8.3.1）
                // - PRACT=0 + PI NS：按 NS metadata 形态分两路：
                //     · separate (meta_inline=false)：B6b host data→PRP / PI→MPTR；
                //     · inline (meta_inline=true)：B6c host 经 dual-PRP 提供完整
                ...
                if !pract && is_pi_capable {
                    ...
                    // then-store-all。**B6c**：inline NS（extended LBA）的 PRACT=0 单 LBA（nlb=1）
                    ...
                    if meta_inline {
                            ...
                            == crate::controller::prp::PrpTier::List
                        {
                            ...
                            // **#4c** PRP1 可带页内偏移（首段=page-O）；PRP2 是 PRP-list 页
                            // 指针，必须页对齐（spec § 4.1.1）。nlb≥2 总传输 ≥8208 恒 List 档，
                            // 无 tier 迁移；gather（NvmWritePrpListFetch）已 offset-aware。
                            if (prp2 & (NVME_PAGE_SIZE - 1)) != 0 {
                                tracing::warn!(
                                    "inline-meta WRITE nlb≥2：PRP2（list 页指针）须页对齐 → PRP_OFFSET_INVALID"
                                );
                                return Some(Cqe::error(
                                    cid,
                                    sq_id,
                            ...
                            let pi_first = ns.pi_first;
                            let prchk = crate::pi::PrChk::from_cdw12(cdw12);
                            // 连续流按 PRP 段切分（首段 = 页大小 − PRP1 偏移，后续整页，末段 partial）：total_pages 个 host 数据页。
                            let prp_off = crate::controller::prp::prp1_offset(prp1);
                            let total_pages =
                                crate::controller::prp::total_pages(prp_off, total_bytes);
                            ...
                            // PRP1 数据页（page 0）：含偏移时首段 = page-O（reviewer #4c）。
                            let first_seg =
                                crate::controller::prp::page_size(0, prp_off, total_bytes);
                        ...
                        // 其它 inline 布局（如 lbads=9+meta=8=520）未覆盖，留待真做时扩展。
                        if block_bytes != NVME_PAGE_SIZE as usize + 8 {
                            tracing::warn!(
                                block_bytes,
                                "inline-meta WRITE 仅支持标准 PI NS（block_bytes=4104）"
                            );
                            return Some(Cqe::error(cid, sq_id, sq_head, phase, sc::INVALID_FIELD));
                        }
                        ...
                        self.pending_ios.insert(
                            ...
                            PendingIo {
                                ...
                                cq_id,
                                nsid,
                                op: PendingOp::InlineMetaWriteSeg {
                                    op_id,
                                    is_prp1: true,
                                },
                            },
                        ...
                        self.inline_meta_writes.insert(
                            op_id,
                            crate::controller::InlineMetaWriteAccum {
                                sq_id,
                                cid,
                                sq_head,
                                cq_id,
                        ...
                        == crate::controller::prp::PrpTier::List
                    {
                        ...
                        // 仅标准 PI NS（lbads=12 + meta=8 → 4104）；其它布局 defer。
                        if block_bytes != NVME_PAGE_SIZE + 8 || data_bytes != NVME_PAGE_SIZE {
                            tracing::warn!(
                                block_bytes,
                                "B6b-4 WRITE 仅支持标准 PI NS（block=4104, data=4096）"
                            );
                            return Some(Cqe::error(cid, sq_id, sq_head, phase, sc::INVALID_FIELD));
                        }
                        // MDTS 按含 metadata 校核（C1①），覆盖前面 nlb*512 下界放行的多 LBA 情况。
                        if (block_bytes * nlb as u64) > MDTS_MAX_BYTES {
                            tracing::warn!(
                                nlb,
                                bytes = block_bytes * nlb as u64,
                                "B6b-4 WRITE 超 MDTS"
                            );
                            return Some(Cqe::error(cid, sq_id, sq_head, phase, sc::INVALID_FIELD));
                        }
                        ...
                        self.pending_ios.insert(
                            ...
                            PendingIo {
                                ...
                                cq_id,
                                nsid,
                                op: PendingOp::NvmWritePrpListSepMeta { op_id },
                            },
                        );
                        return None;
                    }
