...
/// **B6b-4-N>2（separate metadata，PRACT=0，PRP-list-data）差分 oracle** —
/// nlb=3 separate-meta：data 3 页经 PRP1 + PRP-list（PRP2 指向 list 页，含 page2/3 GPA），
/// 3×8 PI tuple 经 MPTR 一条 DMA。
///   WRITE 正例：3 块 host PI 都正确 → backing 3 块 interleave 落盘（逐块独立 round-trip）。
///   WRITE 负例：仅 block 2 host PI guard 损坏 → Media 错误 + **全 3 块都不落盘**（原子）。
///   READ 正例：盘上 3 块 PI 正确 → data 3 页回 PRP1+list / N×8 tuple concat 回 MPTR + success。
///   READ 负例：仅 block 1 stored PI guard 损坏 → 同步 Media 错误 + **不发任何** host DMA-write。
///
/// 独立 oracle：backing file 字节 + PiTuple::compute + capture DmaWrite 字节。
/// revert-verify：把 finalize / dispatch 的 verify 循环改为只查 block 0 → 负例不被拒 →
/// 坏数据落盘 / 回送 host → 原子性断言转红。
#[test]
fn b6b_n_gt_2_prp_list_separate_meta() {
    use pcie_device_core::{CaptureTransport, DeviceCtx, TransportEvent};
    fn sep_ns() -> NvmeController {
        let mut c = make_ctrl_with_tmp("b6b_n3_prp");
        let ns = c.namespaces.get_mut(&1).unwrap();
        ns.lbads = 12;
        ns.meta_size = 8;
        ns.pi_type = 1;
    ...
    // 3 块各 4096 byte data + 各自 tuple。
    let datas: Vec<Vec<u8>> = (0..3)
        .map(|b| {
            (0..4096)
                .map(|i| ((i * (b as usize + 1) * 7 + 11) & 0xff) as u8)
                .collect()
        })
        .collect();
    let tuples: Vec<[u8; 8]> = (0..3)
        .map(|b| crate::pi::PiTuple::compute(&datas[b], b as u64, 1).to_bytes())
        .collect();
    let mut meta_concat = Vec::with_capacity(24);
    for t in &tuples {
        meta_concat.extend_from_slice(t);
    }
    // PRP layout: PRP1=0x4000 (page 0); PRP2=0x9000 → PRP list 页 (含 page1/2 GPA);
    // page1=0x5000, page2=0x6000; MPTR=0x7000 (PI tuple concat).
    let list_page_gpa: u64 = 0x9000;
    let page_gpas: [u64; 3] = [0x4000, 0x5000, 0x6000];
    let mptr_gpa: u64 = 0x7000;
    // PRP list 页内容：2 个 u64 (page1, page2)。
    let mut list_page_bytes = vec![0u8; 4096];
    list_page_bytes[0..8].copy_from_slice(&page_gpas[1].to_le_bytes());
    list_page_bytes[8..16].copy_from_slice(&page_gpas[2].to_le_bytes());
    ...
    fn drive_write(
        ...
        let mut ctx = DeviceCtx::new(cap);
        let mut sqe = io_sqe(0x01, 1, 0, 3, 0x4000, false, cid); // WRITE PRACT=0 nlb=3
        sqe.prp2 = list_page_gpa;
        sqe.mptr = mptr_gpa;
        let r = c.dispatch_io(&mut ctx, 1, sqe, cid, 0, 1);
        assert!(r.is_none(), "N>2 WRITE 走异步 PRP-list 机件");
        // 期望 3 条 pending：list-fetch + PRP1 data + MPTR meta。
        assert_eq!(c.pending_ios.len(), 3);
        assert_eq!(c.prp_list_ops.len(), 1);
        // feed list page（DMA-read）→ 触发 page1/2 dma_read。
        // PendingOp 不可 Clone/Copy → 用 token + 标签的方式而非 (tok, op) 拷贝。
        enum InitTag {
            List,
            Data0,
            Meta,
        }
        let toks_before: Vec<(u64, InitTag)> = c
            .pending_ios
            .iter()
            .map(|(&t, p)| match p.op {
                PendingOp::NvmWritePrpListFetch { .. } => (t, InitTag::List),
                PendingOp::NvmWritePrpListData { page_idx: 0, .. } => (t, InitTag::Data0),
                PendingOp::NvmWritePrpListSepMeta { .. } => (t, InitTag::Meta),
                _ => unreachable!("unexpected initial op"),
            })
            .collect();
        for (t, tag) in toks_before {
            match tag {
                InitTag::List => c.on_dma_complete_impl(&mut ctx, t, true, list_page.to_vec()),
                InitTag::Data0 => c.on_dma_complete_impl(&mut ctx, t, true, datas[0].clone()),
                InitTag::Meta => c.on_dma_complete_impl(&mut ctx, t, true, meta.to_vec()),
            }
        }
        // 此时 list 已 walk + PRP1/meta 已到，page1/2 sub-DMA-read 已 issue。
        let remaining: Vec<(u64, u32)> = c
            .pending_ios
            .iter()
            ...
            .collect();
        for (t, page_idx) in remaining {
            c.on_dma_complete_impl(&mut ctx, t, true, datas[page_idx as usize].clone());
        }
    }
    ...
    // 正例
    {
        ...
        assert!(c.prp_list_ops.is_empty(), "WRITE 完成后 op 清理");
        let ns = c.namespaces.get(&1).unwrap();
        for i in 0..3 {
            let mut buf = vec![0u8; 4104];
            ns.read_at(&mut buf, i as u64 * 4104).unwrap();
            assert_eq!(&buf[0..8], &tuples[i][..], "block {i} PI tuple 落前 8 字节");
            assert_eq!(&buf[8..4104], &datas[i][..], "block {i} data interleaved");
        }
    }

    // 负例（原子性）：block 2 PI guard 损坏 → 全不落盘
    {
        let mut c = sep_ns();
        let mut cap = CaptureTransport::with_start_token(0x100);
        let mut bad_meta = meta_concat.clone();
        bad_meta[16] ^= 0xff; // tuple2 在 meta[16..24]，第 0 字节是 guard 高字节
        let pre = cap.events().len();
        drive_write(
            &mut c,
            &mut cap,
        ...
        let ns = c.namespaces.get(&1).unwrap();
        let mut all = vec![0u8; 4104 * 3];
        ns.read_at(&mut all, 0).unwrap();
        assert!(
            all.iter().all(|&b| b == 0),
            "任一块 PI 失败 → 全 3 块不落盘（含本来正确的 block 0/1）"
        );
        let status = cap
            .events()
            .iter()
        ...
        assert_eq!(
            status,
            crate::cmd::sc::status(0x82, crate::cmd::sc::SCT_MEDIA_DATA_INTEGRITY),
            "block 2 host PI guard 错 → Media GUARD_CHECK_ERR (0x0282)"
        );
    }

    // ============ READ ============
    fn seed_n3(c: &mut NvmeController, datas: &[Vec<u8>], tuples: &[[u8; 8]]) {
        let ns = c.namespaces.get_mut(&1).unwrap();
        for i in 0..3 {
            let mut block = vec![0u8; 4104];
            block[0..8].copy_from_slice(&tuples[i]);
            block[8..4104].copy_from_slice(&datas[i]);
            ns.write_at(&block, i as u64 * 4104).unwrap();
        }
    }
    ...
    {
        let mut c = sep_ns();
        seed_n3(&mut c, &datas, &tuples);
        let mut cap = CaptureTransport::with_start_token(0x100);
        {
            let mut ctx = DeviceCtx::new(&mut cap);
            let mut sqe = io_sqe(0x02, 1, 0, 3, 0x4000, false, 0xA2);
            sqe.prp2 = list_page_gpa;
            sqe.mptr = mptr_gpa;
            let r = c.dispatch_io(&mut ctx, 1, sqe, 0xA2, 0, 1);
            assert!(r.is_none(), "N>2 READ 走异步 PRP-list scatter");
            // 起步：1 条 list-fetch（其它发 scatter 在 walk 完成后）。
            assert_eq!(c.pending_ios.len(), 1);
            // feed list page → 触发 PRP1+page1+page2 scatter + MPTR-write。
            let t_list = *c.pending_ios.keys().next().unwrap();
            c.on_dma_complete_impl(&mut ctx, t_list, true, list_page_bytes.clone());
            // 现在 4 条 DMA-write pending（3 data + 1 meta）。
        ...
        for i in 0..3 {
            assert_eq!(
                writes.get(&page_gpas[i]).map(|d| &d[..]),
                Some(&datas[i][..]),
                "block {i} data → page_gpas[{i}]"
            );
        }
        assert_eq!(
            writes.get(&mptr_gpa).map(|d| &d[..]),
            Some(&meta_concat[..]),
            "N×8 PI tuple concat → MPTR"
        );
        assert!(c.prp_list_ops.is_empty(), "READ 完成后 op 清理");
    }

    // READ 负例：仅 block 1 stored PI guard 损坏 → 同步 Media + 不回送 host
    {
        let mut c = sep_ns();
        let mut bad_tuples = tuples.clone();
        bad_tuples[1][0] ^= 0xff;
        seed_n3(&mut c, &datas, &bad_tuples);
        let mut cap = CaptureTransport::with_start_token(0x100);
        let cqe = {
            let mut ctx = DeviceCtx::new(&mut cap);
            let mut sqe = io_sqe(0x02, 1, 0, 3, 0x4000, false, 0xA3);
            sqe.prp2 = list_page_gpa;
            sqe.mptr = mptr_gpa;
            c.dispatch_io(&mut ctx, 1, sqe, 0xA3, 0, 1)
        };
        let cqe = cqe.expect("block 1 stored PI 损坏 → 同步 Media 错误 CQE");
        assert_eq!(
            cqe_status(&cqe),
            crate::cmd::sc::status(0x82, crate::cmd::sc::SCT_MEDIA_DATA_INTEGRITY)
        );
        let leaked = cap.events().iter().any(|e| matches!(e,
            TransportEvent::DmaWrite { gpa, .. }
                if *gpa == page_gpas[0] || *gpa == page_gpas[1] || *gpa == page_gpas[2] || *gpa == mptr_gpa));
        assert!(
            !leaked,
            "verify-all 失败 → 不应回送 host（含本来正确的 block 0/2）"
        );
    }
...
/// **#4c-a — separate-metadata（PRACT=0，N>2，PRP-list data）PRP1 页内偏移** 差分 oracle。
///
/// spec § 4.1.1：仅 PRP1 可带页内偏移，PRP2（PRP-list 页指针）/list-entry 须页对齐。
/// separate NS 的 data 平面经 PRP（data_bytes=4096），metadata 经 MPTR 单条 DMA。
/// PRP1 偏移 O 把 data 流（nlb×4096）按 host 页重分段——首段 page−O、后续整页、末段
/// partial——与 LBA-data(4096)边界**不再一一对应**，故 controller 必须拼回连续流再按
/// data_bytes 重切 N 块（WRITE finalize）/ 按 host 页重切 scatter（READ）。
///
/// 取 nlb=3、O=0x200：data 流 12288B 分 4 段 [3584,4096,4096,512]（比 O=0 多 1 页）。
///   - WRITE：feed 4 段 → 拼回 → 切 3×4096 → 逐块 verify host PI → interleave 落盘；
///     **独立 oracle 同时断 controller 请求的 DmaRead 段长 == [3584,4096,4096,512]**
///     （封堵 page0 读成固定 4096 的盲点，B6c-3 HIGH 同类）。
///   - READ：盘上 verify-all → 拼回 → 按 host 页重切 scatter；断 DmaWrite 段长 + 重组
///     == data 流、MPTR == tuple concat。
/// revert-verify：把 WRITE page0 读 / READ scatter 重切退回固定 4096 → 段长断言转红。
#[test]
fn b6b_n_gt_2_separate_meta_prp1_offset() {
    use pcie_device_core::{CaptureTransport, DeviceCtx, TransportEvent};
    const O: u64 = 0x200; // 512B 偏移
    const PRP1: u64 = 0x4000 + O;
    const LIST: u64 = 0x9000;
    const PG1: u64 = 0x5000;
    const PG2: u64 = 0x6000;
    const PG3: u64 = 0x8000;
    const MPTR: u64 = 0x7000;
    // 段边界（spec 公式，独立 oracle）：首段 = 4096-O = 3584；中间整页；末段 partial。
    let seg: [usize; 4] = [
        4096 - O as usize,
        4096,
        4096,
        3 * 4096 - (4096 - O as usize) - 4096 - 4096,
    ];
    fn sep_ns() -> NvmeController {
        let mut c = make_ctrl_with_tmp("b6b_sep_off");
        let ns = c.namespaces.get_mut(&1).unwrap();
        ns.lbads = 12;
        ns.meta_size = 8;
        ns.pi_type = 1;
    ...
    // data-only 连续流（separate：metadata 不在流里）。
    let mut stream = Vec::with_capacity(3 * 4096);
    for d in &datas {
        stream.extend_from_slice(d);
    }
    // PRP-list 页：3 个 entry（PG1/PG2/PG3）——偏移使页数从 3 升到 4。
    ...
    list_page[16..24].copy_from_slice(&PG3.to_le_bytes());
    // 段 i 在流中的切片。
    ...
    // ── WRITE：偏移 gather → verify → interleave 落盘 ──
    {
        let mut c = sep_ns();
        let mut cap = CaptureTransport::with_start_token(0x100);
        {
            let mut ctx = DeviceCtx::new(&mut cap);
            let mut sqe = io_sqe(0x01, 1, 0, 3, PRP1, false, 0xC0);
            sqe.prp2 = LIST;
            sqe.mptr = MPTR;
            let r = c.dispatch_io(&mut ctx, 1, sqe, 0xC0, 0, 1);
            assert!(r.is_none(), "偏移 N>2 separate WRITE 走异步 PRP-list");
            enum T {
                List,
                Data(u32),
                Meta,
        ...
        // **独立 oracle（防 page0 固定 4096 盲点）**：data-page DMA-read 段长须 == 偏移分段。
        let mut reads: Vec<(u64, u32)> = cap
            .events()
            .iter()
            .filter_map(|e| match e {
                TransportEvent::DmaRead { gpa, len, .. }
                    if *gpa == PRP1 || *gpa == PG1 || *gpa == PG2 || *gpa == PG3 =>
                {
                    Some((*gpa, *len))
                }
                _ => None,
        ...
        assert_eq!(
            ...
            "WRITE gather 请求段长须含 PRP1 偏移（首段 page-O，非固定 4096）"
        );
        let ns = c.namespaces.get(&1).unwrap();
        for b in 0..3 {
            let mut buf = vec![0u8; 4104];
            ns.read_at(&mut buf, (b * 4104) as u64).unwrap();
            assert_eq!(&buf[0..8], &tuples[b][..], "偏移 sep WRITE block {b} tuple");
            assert_eq!(
                &buf[8..4104],
                &datas[b][..],
                "偏移 sep WRITE block {b} data"
            );
        }
    }

    // ── READ：盘上 verify-all → 偏移 scatter → host 重组 ──
    {
        let mut c = sep_ns();
        {
            let ns = c.namespaces.get_mut(&1).unwrap();
        ...
        {
            let mut ctx = DeviceCtx::new(&mut cap);
            let mut sqe = io_sqe(0x02, 1, 0, 3, PRP1, false, 0xC1);
            sqe.prp2 = LIST;
            sqe.mptr = MPTR;
            let r = c.dispatch_io(&mut ctx, 1, sqe, 0xC1, 0, 1);
            assert!(r.is_none(), "偏移 N>2 separate READ 走异步 scatter");
            let t_list = *c.pending_ios.keys().next().unwrap();
            c.on_dma_complete_impl(&mut ctx, t_list, true, list_page.clone());
            // 4 data scatter + 1 MPTR write。
            assert_eq!(c.pending_ios.len(), 5, "4 data scatter + 1 MPTR write");
            let toks: Vec<u64> = c.pending_ios.keys().copied().collect();
            for t in toks {
                c.on_dma_complete_impl(&mut ctx, t, true, Vec::new());
            }
        ...
        // 段长：PRP1 收首段 page-O，PG1/PG2 整页，PG3 末段。
        assert_eq!(
        ...
        assert_eq!(writes.get(&PG2).map(|d| d.len()), Some(seg[2]), "PG2 整页");
        assert_eq!(writes.get(&PG3).map(|d| d.len()), Some(seg[3]), "PG3 末段");
        let mut got = Vec::new();
        got.extend_from_slice(&writes[&PRP1]);
        got.extend_from_slice(&writes[&PG1]);
        got.extend_from_slice(&writes[&PG2]);
        got.extend_from_slice(&writes[&PG3]);
        assert_eq!(got, stream, "偏移 sep READ 重组 == 盘上 data 流");
        assert_eq!(
            writes.get(&MPTR).map(|d| &d[..]),
            Some(&meta_concat[..]),
            "N×8 PI tuple concat → MPTR"
        );
    }
...
/// **#4c inline nlb≥2 PRP1 偏移（spec § 4.1.1）差分 oracle** —— extended-LBA nlb=2（8208B）
/// 加 PRP1 页内偏移 O=0x200：连续流按偏移分段（首段 3584@PRP1偏移、整页 4096@PG1、末段
/// 528@PG2，首段=page-O），仍 List 档（超过 2 页）。WRITE gather 按段读、finalize 拼回连续
/// 流逐块 verify+存；READ 按段散到 PRP1偏移与 list。
///
/// 独立 oracle：backing 字节（WRITE）/ capture 重组（READ）vs host extended-block 流；
/// 段边界由 spec 公式（首段 page-O）独立算，非读 prp helper。
/// revert-verify：把 io.rs inline nlb≥2 dispatch 的 first_seg/page_size 退回固定 4096 →
/// 偏移下段错位 → backing/重组 != 流 → 转红。
#[test]
fn b6c_inline_nlb_ge_2_prp1_offset() {
    use pcie_device_core::{CaptureTransport, DeviceCtx, TransportEvent};
    const BLOCK: usize = 4104;
    const O: u64 = 0x200; // 512B 偏移
    const PRP1: u64 = 0x4000 + O;
    ...
    const PG1: u64 = 0x5000;
    const PG2: u64 = 0x6000;
    // 段边界（spec 公式，独立 oracle）：首段 = 4096-O = 3584；其后整页；末段 partial。
    let seg: [usize; 3] = [
        4096 - O as usize,
        4096,
        2 * BLOCK - (4096 - O as usize) - 4096,
    ];
    fn inline_ns() -> NvmeController {
        let mut c = make_ctrl_with_tmp("b6c_inl_off");
        let ns = c.namespaces.get_mut(&1).unwrap();
        ns.lbads = 12;
        ns.meta_size = 8;
        ns.pi_type = 1;
    ...
    // ── WRITE：偏移 gather → 落盘 ──
    {
        ...
        {
            let mut ctx = DeviceCtx::new(&mut cap);
            let mut sqe = io_sqe(0x01, 1, 0, 2, PRP1, false, 0xE0);
            sqe.prp2 = LIST;
            let r = c.dispatch_io(&mut ctx, 1, sqe, 0xE0, 0, 1);
            assert!(r.is_none(), "偏移 inline nlb≥2 WRITE 走异步 PRP-list");
            // 起步 feed list + page0；walk 发 page1/2。
            enum T {
                List,
                Data(u32),
            }
        ...
        // **独立 oracle（防 B6c-3 同类盲点）**：controller 请求的 data-page DMA-read 段长
        // 必须 == 偏移分段 [首段 page-O, 整页, 末段]（CaptureTransport 不截断喂入，故 backing
        // 断言不足以抓"page0 读成固定 4096"——必须显式断 DmaRead.len）。
        let mut reads: Vec<(u64, u32)> = cap
            .events()
            .iter()
            .filter_map(|e| match e {
            ...
            .collect();
        reads.sort();
        // 按 gpa 排序：PRP1(0x4200)→首段, PG1(0x5000)→整页, PG2(0x6000)→末段。
        assert_eq!(
            reads,
            vec![
                (PRP1, seg[0] as u32),
        ...
        for b in 0..2 {
            let mut buf = vec![0u8; BLOCK];
            ns.read_at(&mut buf, (b * BLOCK) as u64).unwrap();
            assert_eq!(&buf[0..8], &tuples[b][..], "偏移 WRITE block {b} tuple");
            assert_eq!(&buf[8..BLOCK], &datas[b][..], "偏移 WRITE block {b} data");
        }
    }

    // ── READ：偏移 scatter → host 重组 ──
    {
        let mut c = inline_ns();
        c.namespaces
            .get_mut(&1)
        ...
        {
            let mut ctx = DeviceCtx::new(&mut cap);
            let mut sqe = io_sqe(0x02, 1, 0, 2, PRP1, false, 0xE1);
            sqe.prp2 = LIST;
            let r = c.dispatch_io(&mut ctx, 1, sqe, 0xE1, 0, 1);
            assert!(r.is_none());
            let t_list = *c.pending_ios.keys().next().unwrap();
            c.on_dma_complete_impl(&mut ctx, t_list, true, list_page.clone());
            let toks: Vec<u64> = c.pending_ios.keys().copied().collect();
        ...
        // PRP1(偏移) 收首段 3584；PG1 收 4096；PG2 收 528。重组 == 流。
        assert_eq!(
            writes.get(&PRP1).map(|d| d.len()),
            Some(seg[0]),
            "PRP1 收首段(page-O)"
        ...
        got.extend_from_slice(&writes[&PG1]);
        got.extend_from_slice(&writes[&PG2]);
        assert_eq!(got, stream, "偏移 READ 重组 == 盘上 extended-block 流");
    }
}
