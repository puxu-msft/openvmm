...
| **SGL** (PSDT=10) | ✅ [Q] | segment chain + Bit Bucket（R2d）；**CMB-relative sub_type=1（Offset）**：CMB 启用时**放行 + rebase**（`cba+offset`，CMB-P2/P5）、越界 offset≥size→`SGL_OFFSET_INVALID`(0x16)（P5 严格）、CMB off 时→`SGL_INVALID_USE_OF_CMB`(0x12)；**3 路径经共享 `subtype_to_sc`+`resolve_sgl_address` 一致**；**SGL×PI separate-meta ✅（PRACT 0/1，data 经 SGL frag scatter-gather + PI tuple 经 MPTR 双门控、READ Bit Bucket 跨 LBA 正确丢弃，P-B）；inline-meta ⏳（P-C 暂拒 INVALID_PROTECTION_INFO）** | src/sgl.rs + io.rs SGL 路径 |
