...
- ✅ **P-C 完成**（commit `e686e954c`）：SGL × inline-meta（PRACT 0/1）。inline PRACT=0 = host SGL 携 extended-block（`nlb×block_bytes`，tuple 在流内、无 MPTR、backing 原样存/读）；inline PRACT=1 退化为 data-only（== separate PRACT=1，controller 自算/strip）。**finalize 机件零改动**——复用 P-B `finish_sgl_done` PI 分支：inline `layout Inline` → `is_separate_pi()=false` → 不经双门控直走 finalize → helper `inline_pract0` 分支（verify inline 切轴 + store as-is）。`SglPiArgs.inline` 轴 + dispatch 按 (inline,pract) 分流（pi_sector / op.data interleaved-vs-data_plane / layout / MPTR）。3 个 inline e2e（含 **extended block 跨 fragment** 正交切轴守卫 [2000,2104,4104] 原样落盘）+ revert-verify（flip op.data 选择 → 转红）；rust-reviewer APPROVE 0 C/H（M-1 doc clippy 已修）。SPEC_CONFORMANCE 同步：SGL×PI separate ✅ + inline ✅。
- ✅ **E0 完成**（commit `cf76991c4`）：PSDT/SGL 模型对齐 spec。`resolve_data_pointers` 把 `PSDT∈{01,10}` 合并一臂、data 单/碎由 DPTR descriptor 类型定（与 PSDT 解耦）；dispatch READ/WRITE 加 `meta_sgl=(psdt==0b10)` 2×2 路由（01→P-B/P-C、10+inline→reject、10+separate→E1）；P-B/P-C 测试 `sgl_sqe` PSDT 10→01 迁移 + 5 parse-level oracle + e2e reject oracle。252 lib 绿。**bit 编号订正**：plan §5「bit15」误，实为 **bit19=`NVME_CTRL_SGLS_MSDS`**（E0 rust-reviewer + Linux nvme.h 证伪，见上 §E1 line120 订正）。rust-reviewer APPROVE 0 C/0 H。
- ✅ **E1 完成**（commit `<本次>`）：metadata-SGL（PSDT=10 METASEG）。**POC**：Linux `nvme_pci_setup_meta_iter` 实证 MPTR→meta SGL1 描述符（单 DataBlock=单 contiguous / Last Segment→N DataBlock；无 chain，本实现支持到 `MAX_SGL_SEGMENTS`）。**承重结构**：新 `PiLayout::MetaSgl{mptr}`（不污染 PRP-共享 `Separate`）+ `SglOp` meta 平面字段（`meta_frags/meta_walk_offset/meta_walk_segments/meta_transfers_done/total/meta_walk_done/meta_buf`，与 data 平面平行、独立计数 H-3）+ 新 `PendingOp::NvmSglMetaFetch{Mptr|Segment}`/`NvmSglMetaData` + `finish_sgl_error`/DMA-fail retain 同步清理新变体（防孤儿 token）。data 平面 descriptor-处理抽共享 `append_sgl_data_frags`（meta 复用，§32 anti-drift）。WRITE meta-frag gather→N×8 buf→verify-all；READ tuple concat→meta-frag scatter；门控 `data_done() && (pract || meta_all_done())`。advertise SGLS **bit19**。覆盖≠N×8→`METADATA_SGL_LENGTH_INVALID`(0x10)、MPTR=0→INVALID_FIELD、10+inline→INVALID_FIELD。5 e2e 差分 oracle（WRITE/READ multi-frag + Bit Bucket + MPTR=0 + length-invalid，含 revert-verify）。259 lib 绿 + clippy 0。SPEC_CONFORMANCE：metadata-SGL ✅。
