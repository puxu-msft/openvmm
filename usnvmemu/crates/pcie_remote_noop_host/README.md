...
- `pcie_remote_noop_host_tcp` (`src/main.rs`) — TCP 路径 (OpenVMM dev path)
- `pcie_remote_noop_host_vsock` (`src/vsock_main.rs`) — vsock 路径 (OpenHCL VTL2)
...
[`pcie_remote_userspace_sdk`](../pcie_remote_userspace_sdk/) +
`pcie_remote_nvme_userspace` / `pcie_remote_rng_userspace` 作为模板。
