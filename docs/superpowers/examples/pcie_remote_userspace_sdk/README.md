...
| Consumer | 路径 |
|----------|------|
| **`pcie_remote_nvme_userspace`** | 整个 NVMe controller 教学库 |
| **`pcie_remote_rng_userspace`** | 第二个 PcieDevice 教学 example (最小 RNG) |
| **`nvme_of_tcp_target`** | NVMe-oF TCP target (在 SDK 之上加 NVMe-oF 包装) |
| **vfio-user backend (`pcie_vfio_user_sdk`)** | 让同一份 device 接 QEMU |
...
- 上层教学：[`../pcie_remote_nvme_userspace/README.md`](../pcie_remote_nvme_userspace/README.md)
