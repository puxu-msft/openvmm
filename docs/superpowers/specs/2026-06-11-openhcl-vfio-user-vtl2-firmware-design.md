...
- **W4**：✅ client SET_IRQS eventfd（assign/deassign/clear，多 fd 经 SCM_RIGHTS，loopback IrqTriggerDev doorbell→ctx.fire_interrupt→写 client eventfd→read 验计数++ + 其它向量未误触发）。**全 safe nix EventFd，保持 deny(unsafe_code)**。**scope**：W4 = 中断 firmware→client 半段（eventfd 计数++）；**`Interrupt::deliver`（VTL0 guest 注入）推迟 W6**——需 openvmm/underhill partition + vmcore::interrupt，非 standalone 可测（同 W2/W3 scope ADR）。注：transport spec 表里 W4=Backend::inject_msix 是 server/QEMU-backend 视角，与本 client-side W4 不同。
